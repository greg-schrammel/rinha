...
db.query(
  `CREATE TABLE transactions (userId REFERENCES users(userId), value INTEGER, type TEXT, description TEXT, timestamp DATETIME DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%f', 'NOW')), txId INTEGER PRIMARY KEY);`,
).run()
...
const selectTransactions = db.prepare('SELECT * FROM transactions WHERE userId = $id ORDER BY timestamp DESC LIMIT 10')
